...
/* Moralis init code */
// const serverUrl = "https://vexxy7lhmnmr.usemoralis.com:2053/server";
// const appId = "6YfqznMfzX2KqBZekXMoheY9WYNwnZCXuaXv5j9K";
// Moralis.start({serverUrl, appId});

// let aaaa
// async function getMoralisData() {
//     const data = await Moralis.Cloud.run("getEvents2")
// 	data.forEach(el => {
// 		console.log(el.attributes.addr, parseInt(el.attributes.rawAmount)/1e18, el.attributes.transaction_hash)
// 	})

// 	aaaa = data.attributes
// }
...
function renderMoralisData(data, noattribute) {
    ...
    if (noattribute) {
        ...
        if (data[0]) {
            data[0].forEach(el => {
                txt = `Stake Started: ${((parseInt(el.rawAmount)).toFixed(2) / 1e18).toFixed(0)} IOX for ${el.duration} days`
        ...
        // if (data[3]) {
        // 	data[3].forEach(el => {
        // 		txt = `Auction Collected: ${parseInt(el.rawAmount) / 1e18} IOX`
        // 		dores(el)
        // 	})
        // }
        ...
        if (data[3]) {
            data[3].forEach(el => {
                counter++
                if (counter < 15) {
                    txt = `${(parseInt(el.rawAmount) / 1e18).toFixed(0)} IOX Stake sell offer for ${parseInt(el.price) / 1e18} BNB`
    ...
    // 	if (data[0]) {
    // 		data[0].forEach(el => {
    // 			txt = `Stake Started: ${parseInt(el.attributes.rawAmount) / 1e18} IOX for ${el.attributes.duration} days`
    // 			dores(el)
    // 		})
    // 	}
    ...
    // 	// if (data[3]) {
    // 	// 	data[3].forEach(el => {
    // 	// 		txt = `Auction Collected: ${parseInt(el.attributes.rawAmount) / 1e18} IOX`
    // 	// 		dores(el)
    // 	// 	})
    // 	// }
    ...
    // 			counter++
    // 			if (counter < 15) {
    // 				txt = `${parseInt(el.attributes.rawAmount) / 1e18} IOX Stake sell offer for ${parseInt(el.attributes.price) / 1e18} BNB`
    // 				dores(el)
    // 			}
    // 		})
    // 	}
